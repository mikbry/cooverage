...
import cli, { CooverageCli } from '../src/cli';
...
describe('Cli', () => {
  it('init', async () => {
    ...
    const { version } = getPackage('cooverage');
    expect(stack[0].indexOf(`Cooverage v${version}`) > -1).to.equal(true);
  });
  it('constructor', async () => {
    const pcli = new CooverageCli([], consol, process.exit);
  ...
  it('help', async () => {
    const pcli = new CooverageCli(['--help'], consol, process.exit);
    await pcli.execute(true);
    expect(pcli).to.be.an('object');
    const { version } = getPackage('cooverage');
    expect(stack[0].indexOf(`Cooverage v${version}`) > -1).to.equal(true);
